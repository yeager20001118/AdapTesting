...
Our current release provides both two-sample testing and independence testing under one PyTorch-based interface. Through AdapTesting, we aim to democratize statistical testing in machine learning by providing a unified, efficient, and accessible framework that bridges the gap between theoretical methods and practical applications.
...
## Methods for Independence Testing - referenced paper

- HSICAggInc - [Efficient Aggregated Kernel Tests using Incomplete U-statistics](https://arxiv.org/pdf/2206.09194)
- RDC - [The Randomized Dependence Coefficient](https://papers.nips.cc/paper_files/paper/2013/file/aab3238922bcc25a6f606eb525ffdc56-Paper.pdf)
- FSIC - [An Adaptive Test of Independence with Analytic Kernel Embeddings](https://proceedings.mlr.press/v70/jitkrittum17a/jitkrittum17a.pdf)
...
h, stat_value, p_value = tst(X, Y, device="cuda") # Default method using median heuristic, device can be either "cuda", "cpu" or "mps"

# Other available methods and their default arguments setting (uncomment to use):
# h, stat_value, p_value = tst(X, Y, device="cuda", method="fuse", kernel="laplace_gaussian", n_perm=2000)
# h, stat_value, p_value = tst(X, Y, device="cuda", method="agg", n_perm=3000)
# h, stat_value, p_value = tst(X, Y, device="cuda", method="clf", data_type="tabular", patience=150, n_perm=200)
# h, stat_value, p_value = tst(X, Y, device="cuda", method="deep", data_type="tabular", patience=150, n_perm=200)
...
#### Example usage of Independence Testing

The main API for independence testing is `idt`, which takes paired samples `X` and `Y` with the same number of rows.
...
# Example synthetic paired data
X = torch.randn(1000, 1)
Y = X ** 2 + 0.1 * torch.randn(1000, 1)

# Three currently available independence testing methods:
h, stat_value, p_value = idt(X, Y, device="cuda")  # default method is "rdc"

# Other available methods (uncomment to use):
# h, stat_value, p_value = idt(X, Y, device="cuda", method="fsic", n_perm=200)
# h, stat_value, p_value = idt(X, Y, device="cuda", method="hsicagg", n_perm=500, R=X.shape[0]-1)

"""
Output of idt:
    (result of testing: 0 or 1,
    test statistics value of paired samples,
    p-value of testing)

If testing rejects independence, the console will output
    'Reject the null hypothesis with p-value: ..., the test statistics for {method} is ...'.
Otherwise,
    'Fail to reject the null hypothesis with p-value: ..., the test statistics for {method} is ...'.
"""
...
- Two-sample testing through `tst`
- Independence testing through `idt`

Planned additions:

- Dataset wrappers and examples for independence testing
